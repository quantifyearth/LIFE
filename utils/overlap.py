...
def process_species_worker(
    ...
    while True:
        paths: Optional[List[Path]] = input_queue.get()
        if paths is None:
            break
...
def overlap(
    project_polygon_path: Path,
    species_infos_path: Path,
    output_filename: Path,
    processes_count: int,
) -> None:
    os.makedirs(output_filename.parent, exist_ok=True)

    raw_species_infos = list(species_infos_path.glob("**/historic/*.geojson"))
    print(f"We collected {len(raw_species_infos)} files")

    species_map: Dict[int,List[Path]] = {}
    for species_info in raw_species_infos:
        filename = species_info.name
    ...
    with Manager() as manager:
        ...
        sentinel_counter = 0
        while True:
            val: Optional[Tuple[Path,str]] = result_queue.get()
            if val is None:
                sentinel_counter += 1
