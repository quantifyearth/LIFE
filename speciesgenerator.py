...
def file_writer(file_name = None):
	...
	yield writer
	if file_name: writer.close()

def projectSpeciesList(project: str, ranges: str) -> List[Tuple[int, str]]:
	''' Returns a list of species that have ranges that intersect with project polygon
		To Check: does it give the correct answers? , What do we want the output to be like? (id_nos are float but should maybe be int)
	...
	return [x for x in zip(id_list,binomial_list)]
